function [CRN, N] = f_add_drug_Raf_from_file_deg(CRN, drug)
    
    drug_deg = strcat(drug, '_deg');
    protein_folder = fullfile('../data', sprintf('%s', drug_deg));
    % Files txt with data
    ...
    for i=1:length(file_reactions1{1})
        ...
        strcat([CRN.rates.names{aux_n_par+j}, char(file_reactions1{2}(i))]);
        j = j+1;
        s = s+1;
        if contains(char(file_reactions1{1}(i)), '<->')
            ...
            CRN.reactions.Flux_rate{aux_n_reac+i} = strcat([CRN.reactions.Flux_rate{aux_n_reac+i}, CRN.rates.names{aux_n_par+2},  char(file_reactions1{3}(i))]);
            j = j+1;
            s = s+1;
        end
    end
