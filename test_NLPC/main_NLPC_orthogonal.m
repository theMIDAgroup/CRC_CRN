...
% physiological and mutated stoichiometric surface, using the orthogonal 
% projector (to make a comparison between using the latter or our non 
% linear projector).
% Starting points: x0_all selected with 'main_extract_x0.m'.
% Function used for the simulation: 'f_NLPC_restart' with proj = 1.
% PHYSIOLOGICAL CASE: results in results/'nlpc_ort_phys_testproj.mat'
% MUTATED CASES: results in results/'nlpc_ort_mut_protein.mat', where
...
%% Step 3. Run nlpc with orthogonal projector on the network for physiological cells
if do_phys
...
for ir = 1:n_runs
    ...
    nlpc_ort_phys(ir) = aux_phys;
    clear aux_phys tim_init
end

% 3.4. Save
save(fullfile(folder_results, 'nlpc_ort_phys.mat'), 'nlpc_ort_phys')

clear nlpc_ort_phys x0_all rates_phys S_phys rho_phys n_runs

end
%% Step 4. Run nlpc with orthogonal projector on the network of mutated cells
if do_mutation 
...
for im = 1:n_mutations
    ...
    for ir = 1:n_runs
        ...
        nlpc_ort_mut(ir) = aux_mut;
        clear aux_mut tim_init
    end

    % 4.4. Save
    save(fullfile(folder_results, ...
        sprintf('nlpc_ort_mut_%s.mat', protein)), 'nlpc_ort_mut', 'x0_all')

    clear protein nlpc_ort_mut x0_all rates_mut rho_mut par n_runs
